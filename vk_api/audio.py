...
def scrap_data(html, user_id, filter_root_el=None):
    ...
    for audio in root_el.find_all('div', {'class': 'audio_item'}):
        ...
        try:
            # full_id = tuple(
            #     int(i) for i in RE_AUDIO_ID.search(audio['data-audio']).groups()
            # )
            import ast
            # full_id = tuple(ast.literal_eval(audio['data-audio'])[:2])
            full_id = tuple([int(x.replace('[', '')) for x in audio['data-audio'].split(',')[:2]])
        except:
